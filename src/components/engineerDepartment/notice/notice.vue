<template>
    <div class='notice notice-preview'>
        ...
        <div class="notice-return" v-if="preview">
            <el-button type="info" size="small" @click="returnPublish"><i class="fa fa-hand-o-left"></i> 返回</el-button>
        </div>
    </div>
</template>
...
export default {
    ...
    mounted() {
        ...
        if (announcementId) {
            ...
            this.preview = true
            if (data.updateTime) {
                // let date = new Date(data.updateTime.replace('-', '/'))
