  ...
  <div class="user-manager">
    ...
    <div class="tool-bar">
      ...
      <el-button type="warning"
                 icon="el-icon-edit"
                 size="mini"
                 @click="updateUser">修改用户</el-button>
      <el-button type="info"
      ...
      <el-button type="primary"
                 ...
                 size="mini"
                 style="float:right"
                 @click="dialogVisible = true">角色配置</el-button>
    </div>
    <user-list :users="users"
               v-if="users && users.length>0"
               ref="listuser"
               :headerFlag="true"></user-list>
    <el-dialog title="添加用户"
               :visible.sync="dialogaddVisible">
      <add-user @closeDialog="closeDialog"
                @refactoringUsers="refactoringUsers"
                @refactoringTree="refactoringTree"
                :users="users"
                v-if="dialogaddVisible"></add-user>
    </el-dialog>
    <el-dialog title="更新用户"
      ...
      <add-user @closeDialog="closeDialog"
                ...
                @refactoringTree="refactoringTree"
                :users="users"
                v-if="dialogUpdateVisible"></add-user>
    </el-dialog>
    <el-dialog title="权限配置"
               :visible.sync="dialogVisible"
               width="50%">
      <user-limit :users="users"
                  @closeRolePanel="closeRolePanel"
                  @refactoringUsers="refactoringUsers"></user-limit>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    ...
    refactoringUsers() {
      this.getUsers().then(res => {
        console.log(res)
        let data = res.data
        let users = this.$common.buildTree(data, 'userId', 'parentId')
        this.users = users
      })
    },
    addUser() {
