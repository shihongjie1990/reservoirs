<template>
  <div class='notice-manager'>
    <div class="title page-title"><span>招标信息</span>
      <div class='search-area'>
        <el-input placeholder="请输入标题"
                  suffix-icon="el-icon-search"
                  v-model="searchCondition"
                  @keyup.native.enter="searchNotice">
        </el-input>
      </div>
    </div>
    ...
    <div class="supplement-block"></div>
    <el-table ref="multipleTable"
      ...
      <el-table-column label="标题"
                       ...
                       width='800'>
        <template slot-scope='scope'><a class='notice-a'
             @click="skipTo(scope.row)">{{scope.row.title}}</a></template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getAnnouncements(pagerOp) {
      let thisRequest = this.$http.post(`/api/tenderbid/page?pageNum=${pagerOp.pageNum}&pageSize=${pagerOp.pageSize}`)
      return thisRequest
    },
    skipTo(rowData) {
