<template>
    <div>
        <el-row :gutter="20">
            <el-col :lg="8" :md="8" :xs="24" class="video-operation">
                <div v-for="(item, index) in allChannels" :key="index" class="video-item">
                    <div>{{ item.name }}</div>
                    <el-button type="primary" plain size="mini" v-if="item.channel && item.channel.length > 0" v-for="(channel, i) in item.channel" :key="i" @click="playVideo(channel)"><i class="fa fa-video-camera"></i> {{ "视频" + (i+1) }}</el-button>
                </div>
            </el-col>
            <el-col :lg="16" :md="16" :xs="24">
                <object id="DPSDK_OCX" classid="CLSID:D3E383B6-765D-448D-9476-DFD8B499926D" style="width: 100%; height: 600px" codebase="DpsdkOcx.cab#version=1.0.0.0"></object>
            </el-col>
        </el-row>
    </div>
</template>
