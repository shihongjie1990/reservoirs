<template>
    <div class="month-paper">
        ...
        <div class="operation-block">
            <label>年份：</label>
            <el-date-picker v-model="year" type="year" placeholder="选择年份" :picker-options="pickerOptions" @change="changeDate" value-format="yyyy">
            </el-date-picker>
        </div>
        <div class="month-block" v-if="monthList && monthList.length>0">
            <label>月份：</label>
            <el-button type="info" size="small" plain v-for="(item, index) in monthList" :class="{active: focus == index}" :key="index" @click="selectMonth(item, index)">{{item.month}}月</el-button>
            <!-- <a v-for="(item, index) in monthList" :key="index" @click="selectMonth(item)">{{item.month}}月</a> -->
        </div>
        <el-tabs type="border-card" v-if="projectMonthlyReportId && monthList && monthList.length>0" style="margin-top:20px;">
