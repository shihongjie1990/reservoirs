<template>
  <div class="investment-financing">
    ...
    <div class="tool-operation">
      ...
      <div v-else
           class="apply-tips">提醒：只能在固定时间段内（本月的20号至下个月5号间）申报资金或撤销申报</div>
    ...
    <el-table :data="tableData"
      ...
      <el-table-column label="当前状态"
        ...
        <template slot-scope="scope">
          <span>{{ scope.row.fundApplicationPayDetail | filterStatus }}</span>
        </template>
      </el-table-column>
      <el-table-column label="批复金额"
                       align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.fundApplicationPayDetail | filterActualAmount }}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          ...
          <el-button size="small"
                     ...
                     type="text"
                     v-if="scope.row.statusChn === '未处理'"
                     :class="!isOperation ? 'warnBtn' : ''">撤销</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="详情"
      ...
      <info-detail v-if="infoVisible"
                   @closeDialog="closeDialog"
                   :fundApplicationPayDetail="fundApplicationPayDetail"
...
export default {
  ...
  filters: {
    ...
    filterActualAmount(data) {
      if (data && Object.keys(data).length > 0) {
        return `${data.Actual_amount}${data.Money_unit}`
      } else {
        return ''
      }
    }
  ...
  methods: {
    ...
    viewInfo (data) {
      ...
      this.fundApplicationPayDetail = data.fundApplicationPayDetail
      this.$nextTick(() => {
