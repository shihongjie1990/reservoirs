<template>
    <div>
        ...
        <el-form ref="form" :model="form" label-width="135px" class="tender-add" :rules="rules">
            <el-row :gutter="20">
                ...
                <el-col :lg="12" :md="12" :xs="12">
                    <el-form-item label="招标计划时间：" prop="bidPlanDate">
                        <el-date-picker v-model="form.bidPlanDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss">
                        </el-date-picker>
                    </el-form-item>
                </el-col>
                <el-col :lg="12" :md="12" :xs="12">
                    <el-form-item label="已招标完成时间：" prop="bidCompleteDate">
                        <el-date-picker v-model="form.bidCompleteDate" type="date" placeholder="选择日期" value-format="yyyy-MM-dd HH:mm:ss" :picker-options="pickerOptions">
                        </el-date-picker>
                    </el-form-item>
                </el-col>
                <el-col :lg="12" :md="12" :xs="12">
...
export default {
    data() {
        ...
        pickerOptions.disabledDate = (time) => {
            let timeArr = form.bidPlanDate.split('-')
            let bidPlanDate = timeArr.join('/')
            return time < new Date(bidPlanDate)
    ...
    methods: {
        submitForm() {
            this.$http.post('/api/tender', this.form).then(res => {
                if (res.code === 1002) {
