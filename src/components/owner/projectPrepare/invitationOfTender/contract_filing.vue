<template>
  <div class="tender-manager">
    <div class="title page-title"><span>合同备案</span></div>
    <div class="contract-operation">
      <el-button type="primary"
                 size="mini"
                 @click="dialogaddVisible = true"><i class="fa fa-plus-square-o fa-lg"></i> 新增合同</el-button>
      <el-button type="warning"
                 size="mini"
                 @click="modifyContract"><i class="fa fa-edit fa-lg"></i> 修改合同</el-button>
      <el-button type="info"
                 size="mini"
                 @click="deleteContract"><i class="fa fa-trash-o fa-lg"></i> 删除合同</el-button>
    </div>
      ...
      <el-table-column label="附件"
        ...
        <template slot-scope="scope">
          <el-button size="small"
                     @click="viewFiles(scope.row.contractFiles)"
                     type="text"
                     v-if="scope.row.contractFiles.length > 0">查看</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-dialog title="附件"
    ...
    <el-dialog title="新增合同"
               :visible.sync="dialogaddVisible">
      <add-contract @closeDialog="closeDialog"
                    @getTender="getTender"
                    v-if="dialogaddVisible"></add-contract>
    </el-dialog>
    <el-dialog title="修改合同"
               :visible.sync="dialogmodifyVisible">
      <modify-contract @closeDialog="closeDialog"
                       @getTender="getTender"
                       :modifyData="modifyData"
                       v-if="dialogmodifyVisible"></modify-contract>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    getTender(param) {
      let thisRequest = this.$http.get('/api/contract/own', { loading: { target: '#contractTable' } }).then(res => {
    ...
    deleteContract() {
      if (this.rowData.length === 1) {
        this.$confirm('记录删除后无法恢复，是否继续删除？', '提示', {
          ...
          this.$http.delete(`/api/contract/${id}`, { loading: { operation: true } }).then(res => {
            if (res.code === 1002) {
    ...
    getAllPrice() {
      this.$http.get('/api/contract/calc', { loading: { target: '#contractTable' } }).then(res => {
