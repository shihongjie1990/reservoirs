...
export default {
  ...
  methods: {
    ...
    submitForm() {
      this.$http.put('/api/contract', this.formData, { loading: { operation: true } }).then(res => {
        if (res.code === 1002) {
          this.$message({ message: '合同修改成功', type: 'success' })
          this.$emit('getTender')
  ...
  mounted() {
    this.fileList = this.$common.pushAttachment(this.formData.contractFiles)
  }
}
</script>
