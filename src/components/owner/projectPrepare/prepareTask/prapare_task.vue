<template>
    <div class="prepare-task">
        <div class="prepare-operation">
            <el-button type="primary" size="mini" @click="dialogaddVisible = true"><i class="fa fa-plus-square-o fa-lg"></i> 新增计划</el-button>
            <el-button type="warning" size="mini" @click="modifyPlan"><i class="fa fa-edit fa-lg"></i> 修改计划</el-button>
        </div>
        <el-table :data="tableData" border style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="planProject" label="计划项目">
            </el-table-column>
            <el-table-column prop="compileUnit" label="编制单位">
            </el-table-column>
            <el-table-column prop="referenceNumber" label="文号">
            </el-table-column>
            <el-table-column prop="approvalUnit" label="批复单位">
            </el-table-column>
            <el-table-column prop="approvalDate" label="批复时间" :formatter="formatterDate">
            </el-table-column>
            <el-table-column label="附件" :formatter="formatterFiles">
                <template slot-scope="scope">
                    <el-button size="small" @click="viewFiles(scope.row.preProgressEntryFiles)" type="text" v-if="scope.row.preProgressEntryFiles.length > 0">查看</el-button>
                </template>
            </el-table-column>
            <el-table-column label="批复状态" align="center">
                <template slot-scope="scope">
                    <el-tag size="medium" v-if="parseInt(scope.row.approvalStatus) === 0" type="info">办理中</el-tag>
                    <el-tag size="medium" v-if="parseInt(scope.row.approvalStatus) === 1" type="success">通过</el-tag>
                    <el-tag size="medium" v-else-if="parseInt(scope.row.approvalStatus) === 2" type="danger">驳回</el-tag>
            ...
            <el-table-column prop="submitter" label="更新人">
            </el-table-column>
        </el-table>
        <el-dialog title="添加计划" :visible.sync="dialogaddVisible">
            <add-task @closeDialog="closeDialog" @getAllNodes="getAllNodes" v-if="dialogaddVisible"></add-task>
        </el-dialog>
        <el-dialog title="修改计划" :visible.sync="dialogmodifyVisible">
...
export default {
    ...
    methods: {
        getAllNodes() {
            this.$http.get('/api/preprogress/possessed').then(res => {
                if (res.code === 1002) {
