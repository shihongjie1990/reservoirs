<template>
    <div class="paper-detail content-item">
        <div class="detail-base">
            ...
            <table class="content">
                ...
                <!-- <tr>
                    <td colspan="4"><label>月报进度：</label>
                        <el-progress :text-inside="true" :stroke-width="18" :percentage="100" status="success"></el-progress>
                    </td>
                </tr> -->
            </table>
            <div class="attachment-files" v-if="attachmentData">
                <label>附件：</label>
                <div class="attachment-block">
                    <i v-for="(item, index) in attachmentData" :key="index" :class="'fa ' + item.icon" :title="item.name" @click="downloadFile(item.fileAddr)"></i>
                </div>
            </div>
        </div>
        <div class="detail-content centerline-panel panel-mini-font">
            <el-collapse v-model="activeNames">
                <el-collapse-item title="投资情况" name="1">
                ...
                <el-collapse-item title="工程量情况" name="2" class="">
                    <div class="detail-investment info-item">
                        <div>
                            <table class="detail-info-desc">
                                <tr>
                ...
                <el-collapse-item title="项目情况" name="3">
                    <div class="detail-investment info-item">
                        <div>
...
export default {
    ...
    methods: {
        getProjectMonthlyReportByProjectMonthlyReportid(projectMonthlyReportId) {
            let thisRequest = this.$http.get(`/api/pmr/${projectMonthlyReportId}`)
            return thisRequest
        },
        getFiles(files) {
            files.map(item => {
    ...
    mounted() {
        // let projectMonthlyReportId = '2c9180836513b2b6016513b3be3f0000'
