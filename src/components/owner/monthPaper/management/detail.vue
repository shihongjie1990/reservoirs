    ...
    <div class="detail-base">
      ...
      <div class="attachment-files"
        ...
        <div class="attachment-block">
          <i v-for="(item, index) in attachmentData"
             :key="index"
             :class="'fa ' + item.icon"
             :title="item.name"
             @click="downloadFile(item.fileAddr)"></i>
        </div>
      </div>
    </div>
    <div class="detail-content centerline-panel panel-mini-font">
      <el-collapse v-model="activeNames">
        <el-collapse-item title="投资情况"
                          name="1">
  ...
  methods: {
    ...
    getFiles(files) {
      files.map(item => {
        ...
        item.name = url.substring(url.lastIndexOf('-') + 1)
        return item
      })
      return files
    },
